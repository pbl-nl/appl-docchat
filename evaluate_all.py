...
def main(chunk_size=None, chunk_overlap=None, chunk_k=None):
    ...
    with open(os.path.join(settings.EVAL_DIR, settings.EVAL_FILE_NAME), mode='r', encoding='utf8') as eval_file:
        eval_file = json.load(eval_file)
    folder_list = eval_file.keys()
