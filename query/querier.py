...
class Querier:
    ...
    def ask_question(self, question: str, *args) -> Dict[str, Any]:
        """
        ...
        # Detects languages and translates question
        questions = None # In case "no args", response can take question instead of QUESTIONS
        if args:
            languages = ut.retrieve_languages_from_vector_store(content_folder_name=args[0], embeddings_model=self.embeddings_model)
            # Set up the Azure OpenAI instance with credentials
            llm = LLMCreator(self.llm_provider, self.llm_model).get_llm()
            questions = []
            for language in languages:
                translation = llm.predict(f"Translate the following text to {language} and only return translation: \n {question}")
                questions.append(translation)
            # to ensure that response is in user's language 
            language = ut.LANGUAGE_MAP.get(ut.detect_language(text=question), 'english')
            questions = ' '.join(questions) + f'\n answer above question in {language} language'
            logger.info(f"Translations: {questions}")

        # (questions or question) is required in case of LLM has no source document then
        # response can be made with below if-statement
        response = self.chain.invoke({"question": (questions or question), "chat_history": self.chat_history})
