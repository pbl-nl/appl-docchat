    ...
    def ask_question(self, question: str, *args) -> Dict[str, Any]:
        ...
        if args:
            ...
            for language in languages:
                translation = self.llm.invoke(f"Translate the following text to {language} and only return translation: \n {question}")
                chunks.extend(self.retriever.invoke(translation))
            context = sorted(chunks, key=lambda doc: doc.metadata['relevance_score'], reverse=True)[:min(self.chunk_k, len(chunks))]
            self.chain.context = context
